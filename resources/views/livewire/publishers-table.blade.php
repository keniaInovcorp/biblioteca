    ...
    <div class="card bg-base-100 shadow-xl overflow-x-auto">
        <div class="card-body p-0">
            <div class="px-3 py-3 flex items-center justify-between gap-3 flex-wrap">
                <h2 class="text-lg font-semibold">Editoras</h2>
            ...
            <table class="table table-zebra table-sm">
                ...
                <tbody>
                        ...
                        <tr>
                            <td>
                                @if($publisher->logo_url)
                                    <img class="size-10 rounded-box object-cover" src="{{ $publisher->logo_url }}" alt="{{ $publisher->name }}" />
                                @else
                                    <div class="avatar placeholder">
                                        <div class="bg-primary text-primary-content rounded-box size-10">
                                            <span class="text-sm font-bold">{{ substr($publisher->name, 0, 1) }}</span>
                                        </div>
                                    </div>
                                @endif
                            </td>
                            <td class="align-middle">
                                <div class="font-semibold">{{ $publisher->name }}</div>
                                <div class="text-xs opacity-60">ID: {{ $publisher->id }}</div>
                            </td>
                            <td class="align-middle">{{ $publisher->created_at->format('d/m/Y H:i') }}</td>
                            <td class="align-middle">
                                <div class="flex w-28 justify-center gap-1 ml-auto">
                                    <a class="btn btn-square btn-ghost btn-sm" href="{{ route('publishers.show', $publisher) }}" aria-label="Ver">
                                        <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    @auth
                                    <a class="btn btn-square btn-ghost btn-sm" href="{{ route('publishers.edit', $publisher) }}" aria-label="Editar">
                                    ...
                                    <form action="{{ route('publishers.destroy', $publisher) }}" method="POST" class="inline-flex">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn btn-square btn-ghost btn-sm text-error hover:bg-error hover:text-white" onclick="return confirm('Eliminar {{ $publisher->name }}?')" aria-label="Eliminar">
                    ...
                    @empty
                        <tr>
                            <td colspan="4">
                                <div class="py-10 text-center text-sm opacity-60">Nenhuma editora encontrada.</div>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
