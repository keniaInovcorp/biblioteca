                        ...
                        <li class="list-row hover:bg-base-50 transition-colors">
                            ...
                            <div>
                                @if($book->cover_image_url)
                                    <img class="size-12 rounded-box object-cover" src="{{ $book->cover_image_url }}" alt="{{ $book->name }}"/>
                                @else
                                    <div class="avatar placeholder">
                                        <div class="bg-primary text-primary-content rounded-box size-12">
                                            <span class="text-lg font-bold">{{ substr($book->name, 0, 1) }}</span>
                                        </div>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="flex-1">
                                ...
                                <div class="text-xs opacity-60 flex items-center gap-2 flex-wrap">
                                    <span class="badge badge-ghost badge-xs">ISBN: {{ $book->isbn }}</span>
                                    <span>•</span>
                                    <span>Editora: {{ $book->publisher->name }}</span>
                                    @if($book->authors->count() > 0)
                                        <span>•</span>
                                        <span>Autores: {{ $book->authors->pluck('name')->join(', ') }}</span>
                                    @endif
                                    @if($book->price)
                                        <span>•</span>
