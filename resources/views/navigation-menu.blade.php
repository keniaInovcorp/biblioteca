        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                @if(Auth::user()?->hasRole('citizen'))
                    @php
                        $itemCount = app(\App\Services\CartService::class)->getItemCount(Auth::user());
                    @endphp
                    <x-nav-link href="{{ route('cart.index') }}" :active="request()->routeIs('cart.*')" class="ms-3 relative">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                        </svg>
                        @if($itemCount > 0)
                            <span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">{{ $itemCount }}</span>
                        @endif
                    </x-nav-link>
                @endif
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()?->hasRole('citizen'))
                <x-responsive-nav-link href="{{ route('cart.index') }}" :active="request()->routeIs('cart.*')">
                    ...
                    @endphp
                    @if($itemCount > 0)
                        <span class="bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">{{ $itemCount }}</span>
                    @endif
                </div>
            </x-responsive-nav-link>
            @endif
