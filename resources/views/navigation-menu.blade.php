    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <!-- Cart Button, only for citizens -->
                @if(Auth::user()?->hasRole('citizen'))
                    @php
                        $cartCount = app(\App\Services\CartService::class)->getItemCount(Auth::user());
                    @endphp
            ...
            @if(Auth::user()?->hasRole('citizen'))
                <x-responsive-nav-link href="{{ route('cart.index') }}" :active="request()->routeIs('cart.*')">
                    <div class="flex items-center justify-between w-full">
                        ...
                        @php
                            $cartCountMobile = app(\App\Services\CartService::class)->getItemCount(Auth::user());
                        @endphp
                        @if($cartCountMobile > 0)
                            <span class="bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">{{ $cartCountMobile }}</span>
                        @endif
                    </div>
                </x-responsive-nav-link>
            @endif
